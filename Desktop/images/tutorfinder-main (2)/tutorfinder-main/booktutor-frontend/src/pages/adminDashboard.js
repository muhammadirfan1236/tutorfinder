...
const AdminDashboard = () => {
    ...
    const studetnsData = () => {
        axios.get(`${process.env.REACT_APP_BASE_URL}/api/students/allStudents`).then
            ((res) => {
                console.log("abc", res)
    ...
    const handleDeleteStudent = async () => {
        try {
          const response = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/students/deleteStudent/${studentId}`);
      ...
      const handleDeleteTeacher = async () => {
        try {
          const response = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/teachers/deleteTeacher/${teacherId}`);
          console.log(response.data); // Handle success message or data
        //   onDelete(); // Call the callback function to handle any necessary UI updates
            ...
            <section id="content">
              ...
              <div class="order container">
                  <div class="head">
                   ...
                   { studentToggle &&  <div className='d-flex gap-2 align-items-center'>
                        <h5 className="mb-0">Are you sure you want to delete this student</h5>
                        <button className='btn btn-primary' onClick={handleDeleteStudent}>Yes</button>
                        <button className='btn btn-secondary' onClick={() => setStudentToggle(false)}>No</button>
                     </div>}
                  </div>
                      ...
                      {students?.map((item, index) => (
                              <tr>
                                  <td className='d-flex gap-3 align-items-center'>
                                      <img src={item?.image} alt="" style={{width:"80px" , height:"80px" , borderRadius:"8px"}} />
                                      <p>{item.name}</p>
                                  </td>
                                  <td>{item.email}</td>
                                  <td>{item.gender}</td>
                    ...
                    <div class="table-data">
                        <div class="order container">
                            <div class="head">
                              ...
                              { teacherToggle &&  <div className='d-flex gap-2 align-items-center'>
                        <h5 className="mb-0">Are you sure you want to delete this teacher</h5>
                        <button className='btn btn-primary' onClick={handleDeleteTeacher}>Yes</button>
                        <button className='btn btn-secondary' onClick={() => setTeacherToggle(false)}>No</button>
                     </div>}
